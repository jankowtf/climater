...
model_estimate_distances_v2 <- function(
  ...
  dat_dist <- lapply(1:nrow(dat_input), function(row_user) {
    ...
    ## SEB
    #dat_distance_geo_mod <- data.frame(
    #  dim_station = unique(dat_station$dim_station),
    #  msr_distance_mod = NA
    #)
    ...
    if (all(cols %in% colnames(dat_input_row))) {
      ...
      #  select(dim_station, msr_distance_mod) %>%
      #  group_by(dim_station) %>%
      #  summarise(msr_distance_mod = unique(msr_distance_mod)) %>%
      #  ungroup()
    }
    dat_db_mod <- dat_db_mod %>%
      select(-matches("dim_")) %>%
      as.matrix()
#####
  ...
  #  length(dat_input_row)       # nothing....
  #  class(dat_input_row)        # nothing....
  #  attributes(dat_input_row)   # nothing....
  #  typeof(dat_input_row)       # nothing....
  #  colnames(dat_input_row)     # nothing....
  #      print(dat_input_row)     # 7
    #print("hello")    # wird angezeigt, sobald man ShinyApp auch bedient
        ####
...
###
      ...
      ### Seb:
     ret <- lapply(1:nrow(dat_db_mod), function(row) {
     dat_db_this <- dat_db_mod[row, colnames(dat_input_row), drop = FALSE]   # selektiert Spalten
      ###


      ## dat_db_this is current station in the loop?!

      # List availale distance measures
      # philentropy::getDistMethods()
     #print(row) # nothing
      ## input row is just an index?! need modify dat_db_this ?!
#      length(dat_db_this)       # nothing....
#      class(dat_db_this)        # nothing....
#      attributes(dat_db_this)   # nothing....
#      typeof(dat_db_this)       # nothing....
#      colnames(dat_db_this)     # nothing....
#      print(dat_db_this)     # nothing
      #print("hello")    # wird angezeigt, sobald man ShinyApp auch bedient
      ####
